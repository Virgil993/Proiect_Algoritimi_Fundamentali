...
vector<long long> Graf_Orientat_Cost::Dijkstra()
{
    int numar_noduri = Graf::get_nr_noduri();//retinem numarul de noduri pentru a nu apela getterul de prea multe ori
    long long infinit=1061109567;//retinem un cost maxim ce nu poate fii atins
    vector<long long> noduri_cost(numar_noduri+1,infinit);//vectori ce retine pentru fiecare nod distanta de la 1 la el, il initializam pe tot cu infinit
    vector<int> viz(numar_noduri+1,0);//vectorul ce retine daca un nod a fost selectat deja sau nu
    noduri_cost[1]=0;//drumul de la 1 la 1 este 0
    viz[1]=1;//nodul 1 este deja vizitat
    ...
    if(lista_adiacenta.find(1)!=lista_adiacenta.end())//parcurgem toti vecinii lui 1
        for(int i=0; i<lista_adiacenta[1].size(); i++)
        {
            noduri_cost[lista_adiacenta[1][i].first]=lista_adiacenta[1][i].second;//actualizam drumurile fiecarui vecin
            pair<int,int> cost_nod;//cream pentru fiecare vecin o pereche de forma (cost,nod) si o introducem in heap
            cost_nod.first=lista_adiacenta[1][i].second;
            cost_nod.second=lista_adiacenta[1][i].first;
