...
{
    //in aceasta functie vom face o parcurgere in adancime in care retinem si diametrul maxim
    int d_max=contor;
    viz[start]=1;
...
{
    //in aceasta functie vom face mai intai o parcurgere BFS pentru a ajunge la cea mai indepartata frunza fata de radacina
    //apoi vom apela functia DFS pentru a calcula diametrul maxim intre aceasta frunza si cea mai indepartata frunza
...
{
    //aceasta functie este exact la fel cu BFS_DFS_DARB cu exceptia ca este o functie statica pentru a scadea timpul de executie
    //intrucat listele de adiacenta cu unordered_map sunt mai ineficiente decat listele de adiacenta cu vector<vector>
...
{
    //aceasta este o functie identica cu DFS_DARB cu exceptia faptului ca este statica pentru a mari timpul de executie
    int d_max=contor;
    viz[start]=1;
...
{
    //aceasta este functia statica apealata pentru a citi si a retine intr-o structura vector<vector> datele despre graf
    //care de asemenea afiseaza si rezultatul in fiserul de iesire
...
public:
    ...
    string tip_graf();
    virtual ifstream& citire_graf_virtuala_fisier(ifstream& in);
    //functia de citire pentru matricea data la EX ROY FLOYD
    void citire_graf_roy_floyd(string nume_fisier);
    //intrucat exercitiul ROY FLOYD se rezolva retinand graful intr-o matrice , o optiune de a rezolva exercitiul cu liste de adiacenta este sa apelam Dijkstra de n ori, cu toate ca este ineficient
    //de asemenea la Graful Hamilton este mai eficient daca retinem listele de adiacenta in vector<vector> decat in unordered map
    //de aceeast am creat doua functii statice ce vor citii si vor afisa rezultatele necesare pentru a fi luate in considerare pe INFO ARENA
...
{
    //in aceasta functie vom rezolva dinamic cu ajutorul matricei[conf][nod_final] ce va retine distanta minima de la nod_inceput la nod_start ce trece prin nodurile prezente in conf
    //in acest caz configuratia este un numar in baza 2 in care un bit de 1 pe pozitia i inseamna ca drumul trece prin nodul i (respectiv un bit de 0 pe pozitia i inseamna ca drumul nu trece prin nodul i)
...
{
    //aceasta este functia statica ce va rezolva EX Hamilton
    ifstream fin(fisier_in);
    ofstream fout(fiser_out);
    vector<vector<int>> liste_ad_inversa(20);//Lista de adiacenta ce retine pentru fiecare nod, toate nodurile ce au o muchie ce duce in nodul curent, vom face acest lucru deoarece , cu ajutorul programarii dinamice
    //noi vom porni de la final si vom umple matricea cu valori in mod invers,cu alte cuvinte , toate drumurile pe care vom vrea sa le calculam , le vom calcula recursiv, deci pentru a fi mai eficienti, vom folosi
    //aceasta lista de adiacenta inversa pentru ajunge in maxim O(n) la urmatorul nod din drum
    vector<vector<int>> costuri(20,vector<int>(20));//matricea in care vom tine costurile
    vector<vector<int>> matrice(262150,vector<int>(20,-1));//matricea in care vom retine toate valorile drumurilor minime din graf (acestea fiind spuse, vom retine doar valorile necesare pentru a calcula un ciclu hamiltonian minim)
    int n,m;
    fin>>n>>m;
    for(int i=0;i<m;i++)
    {
    ...
    matrice[1][0]=0;//deoarece noi cautam un ciclu, nu conteaza care este nodul de inceput, asa ca vom calcula nodul de start ca 0 si vom initializam matricea de la 0 la 0 ca fiind distanta 0
    for(int i=0;i<liste_ad_inversa[0].size();i++)//vom parcurge toate nodurile ce au o muchie ce vine in 0 si vom calcula recursiv distanta minima de la 0 la ele , si vom pastra doar minimul
    ...
    if(solutie==100000000)//daca solutia nu s-a schimbat atunci inseamna ca nu am gasit niciun ciclu, deci nu exista solutie
...
{
    //aceasta este functia statica ce va rezolva Exercitiul Roy Floyd
    ifstream fin(fisier_in);
    ofstream fout(fisier_out);
    int INF = 1061109567;
    int n;
    fin>>n;
    int matrice[101][101];//vom pastra in aceasta matrice[x][y] costul drumului de la x la y
    int dis_minima[101][101];//aceasta va fi matricea ce va fi afisata la final in fisier
    for(int i=0;i<n;i++)
        for(int j=0;j<n;j++)
        {
            fin>>matrice[i][j];
            if(i!=j && matrice[i][j]==0)//citim matricea, si nu luam in seama muchile ce se stau in acelasi nod, intrucat toate muchile sunt pozitive si noi cautam un drum minim
                dis_minima[i][j]=INF;
            else
                dis_minima[i][j]=matrice[i][j];//de asemenea intializam si matricea de drumuri minime

        }
    for(int i=0;i<n;i++)//nodurile i vor fi nodurile intermediare pe care stim ca {0,..i-1} toate drumurile au fost setate, asa ca folosim aceste drumui pentru a calcula si drumurile ce il au ca intermediar pe i
        for(int j=0;j<n;j++)//acestea vor fi nodurile de start si nodurile de final vor fi k
            for(int k=0;k<n;k++)//acestea sunt nodurile de final, iar noi vom verifica pentru fiecare daca nodul i ca intermediar va face imbunatatiri la drumurile minime
    {
    ...
    //la final afisam matricea si daca distanta minima este infint atunci afisam 0
    for(int i=0;i<n;i++)
    {
        for(int j=0;j<n;j++)
        {
...
{
    //aceasta functie este folosita pentru a citi si a retine in listele de adiacenta unordered map informatii despre graf
    //Observatie!!
    //aceasta functie este folosita doar in cazul in care vrem sa rezolvam problema matricei drumurilor minime apeland Djsiktra de n ori
...
private:
    ...
    //functii ajutatoare pentru a  afla fluxul maxim dintr-un graf
...
{
    //aceasta functie va crea un lant nesaturat cu un BFS
    for(int i=1;i<=Graf::get_nr_noduri();i++)//mai intai initializam tot vectorul de tati cu 0
        tata[i]=0;
    int viz[1001]={0};
    queue<int> coada_noduri;
    viz[1]=1;
    coada_noduri.push(1);//adaugam in coada primul nod
    while(!coada_noduri.empty())
    {
        int nod = coada_noduri.front();
        coada_noduri.pop();//eliminam nodul din coada
        for(int i=0;i<muchii[nod].size();i++)//parcurgem toate muchie ce pornesc din nodul curent
        {
            int nod2 = muchii[nod][i];
            if(viz[nod2]==0)//daca nodul urmator nu este vizitat
            {
                //daca muchia ce la urmatorul nod este nesatura
                ...
                //sau daca muchia ce duce la urmatorul nod este reziduala si fluxul nu este 0(adica mai avem unitati de luat de pe pe acest drum rezidual)
                if(capacitate[nod][nod2]==0 && flux[nod2][nod]>0)
                {
                    coada_noduri.push(nod2);
                    viz[nod2]=1;
                    tata[nod2]=-nod;//daca muchia este reziduala atunci vom retine tatal ca fiind valoare nodului negativa
                    if(nod2==Graf::get_nr_noduri())//daca am ajuns la final inseamna ca avem un lant nesaturat valid si returnam true si vom avea lantul prin intermediul vectorului de tati
                        return true;
                }
            }
        }
...
{
    //in aceasta functie vom restructura fluxul graful pentru a fi up to date
    int minim=110001;
    int n=Graf::get_nr_noduri();
    while(tata[n]!=0)//mai intai parcurgem tot lantul pentru a afla fluxul minim
    {
        if(tata[n]>0)
        {
    ...
    //a doua oara parucrgem graful pentru restructura graful cu fluxul corect
    //daca e o muchie normala atunci adugam minimul
    //daca e o muchie reziduala atunci scadem minimul
    while(tata[n]!=0)
    {
        if(tata[n]>0)
        {
...
int Graf_Orientat_Flux::Max_flow()
{
    ...
    for(int i=0;i<1000;i++)//intializam matricea de flux cu 0
    {
        flux[i]=new int[1001];
        for(int j=0;j<1000;j++)
            flux[i][j]=0;
    }
    //cat timp putem construcii un lant nesaturat
    //atunci restructuram graful cu fluxul corect
    //daca nu mai putem sa construim un lant nesaturat
    //inseamna ca avem fluxul minim
    while(lant_nesat_BF(tata,flux))
        revizuieste_flux_lant(tata,flux);
    int suma=0;
    for(int i=0;i<muchii[1].size();i++)//calculam fluxul minim ca fiind suma fluxurilor muchiilor ce ies in nodul 1
    {
        int nod=muchii[1][i];
...
{
private:
    //in lista de adiacenta vom retine pentru fiecare lista_ad[nod] toate nodurile ce sunt legate de nod
    vector<int> liste_ad[85000];

    //in lista_nod_muchii vom retine pentru fiecare lista_nod_muchii[nod] toate muchile ce sunt legate de nod (astfel incat pozitia i din lista_ad[nod] este nodul la care se ajunge cu muchia i din lista_nod_muchii[nod])
    //acest lucru este necesar pentru a elimina muchii din graf atunci cand nu mai avem nevoie de ele
...
{
    //in aceasta functie vom face o parcurgere dfs dar vom verifica daca muchiile au fost vizitate
    while(!liste_ad[nod].empty())//cat timp nodul curent are noduri adiacente
    {
        int final_vector=liste_ad[nod].size()-1;//vom lua ultimul element din vector pentru a putea sa eliminam un element in O(1)
        int nod_urmator=liste_ad[nod][final_vector];//retinem urmatorul nod
        int muchie_urmatoare=liste_nod_muchii[nod][final_vector];//retinem urmatoarea muchie

        liste_ad[nod].pop_back();//eliminam atat nodul cat si muchia respectiva
        liste_nod_muchii[nod].pop_back();
        if(viz[muchie_urmatoare]==0)//daca muchia nu a fost vizitata, o parcurgem si continuam dfs cu urmatorul nod
        {
            viz[muchie_urmatoare]=1;
            DFS_Euler(nod_urmator,viz,solutie);
        }
    }
    solutie.push_back(nod);//la final se adauga la solutie nodul curent (in vectorul solutie va fi returnat ciclu eulerian )
}
vector<int> Multigraf::Ciclu_Euler()
{
    int nr_noduri2= Graf::get_nr_noduri();
    vector<int> muchii_vizitate(500005,0);//vectorul viz
    vector<int> solutie;
    for(int i=1;i<nr_noduri2;i++)
        if(liste_ad[i].size()%2!=0)//daca exista un nod care are un numar impar de vecini inseamna ca nu exista solutie
    {
        solutie.push_back(-1);
        return solutie;
    }
    //daca exista solutie apelam DFS_euler si returnam vectorul solutie
...
class Graf_Bipartit
{
//aceasta clasa reprezinta un Graf Bipartit cu nr_noduri_1 in stanga si nr_noduri_2 in dreapta ce va fi retinut intr-o matrice[i][j] = 1 ce inseamna ca exista o muchie de la nodul i din stanga la nodul j din dreapta
private:
...
{
    //in aceasta functie vom crea un drum augemntat prin un dfs care merge o data prin o muchie unmatched de la stanga la dreapta si o data printr-o muchie machted de la dreapta la stanga
    vector<int> solutie;
    if(check==0)//daca suntem in partea stanga
    {
        viz_stanga[nod]=1;//notam nodul ca fiind vizitat
        int ok=1;
        for(int i=1;i<=this->nr_noduri_2;i++)
            if(this->matrice[nod][i]==1 && mat[nod][i]==0 && viz_dreapta[i]==0)//parcurgem toata nodurile vecine ale nodului curent si verificam daca muchia este unmatched si daca nodul nu este vizitat
        {
            cout<<"\nVecinul lui "<<nod<<" este "<<i<<"\n";
            solutie=DFS_Cuplaj(i,1,viz_dreapta,viz_stanga,right_match,mat);//daca sunt respectate conditiile de mai sus atunci parcurgem verificam daca avem o solutie valida continuand cu DFS
            if(solutie.size()!=0)//daca nu este vectorul vid inseamna ca exista o solutie valida, asa ca adaugam nodul curent si returnam solutia
            {
                solutie.push_back(nod);
                return solutie;
            }
        }
        //daca nu am returnat nimic inseamna ca nu avem solutii valide pornind din acest nod asa ca returnam sirul vid
        vector<int> vid;
        return vid;
    }
    else//daca suntem pe partea dreapta
    {
        viz_dreapta[nod]=1;//notmal nodul ca fiind vizitat
        int ok=1;
        if(right_match[nod]==0)//daca nodul nu este matched inseamna ca putem sa ne oprim deoarece am ajuns la o solutie valida
        {
            solutie.push_back(nod);
            return solutie;
        }
        else//daca nodul este matched
        {
            for(int i=1;i<=this->nr_noduri_1;i++)
                if(this->matrice[i][nod]==1 && mat[i][nod]==1 && viz_stanga[i]==0)//verificam daca avem o muchied matched ce se duce intr-un nod nevizitat
            {
                cout<<"\nVecinul lui "<<nod<<" este "<<i<<"\n";
                solutie=DFS_Cuplaj(i,0,viz_dreapta,viz_stanga,right_match,mat);//daca avem o astfel de muchie continuam DFS
                if(solutie.size()!=0)//daca a fost gasita o solutie valida o returnam dupa ce adaugam nodul curent in vector
                {
                    solutie.push_back(nod);
                    return solutie;
                }
            }
            //daca nu a fost gasit nicio solutie valida asta inseamna ca returnam sirul vid
            vector<int> vid;
            return vid;
        }
    }
...
{
    //variabila contor va retine cate muchii se afla in acest cuplaj maxim
    vector<vector<int>> solutie(this->nr_noduri_1+1,vector<int>(this->nr_noduri_2+1,0));//matricea in care vom retine cuplajul maxim
    vector<int> Right_match(this->nr_noduri_2+1,0);//vectorul ce verfica daca nodurile din dreapta sunt sau nu matched
    vector<int> viz(this->nr_noduri_1+1,0);//acest vector va verifica daca nodurile din stanga trebuie sa fie verificate daca au un drum augumentat ce porneste din ele
    while(true)//avem o bucla infinita
    {
        vector<int> viz_right(this->nr_noduri_2+1,0);//vectorul cu care verificam daca nodurile din dreapta sunt vizitate
        vector<int> viz_left(this->nr_noduri_1+1,0);//vectorul cu care verificam daca nodurile din stanga sunt vizitate
        int nod_start=-1;
        for(int i=1;i<=this->nr_noduri_1;i++)//cautam un nod din stanga unmatched
            if( viz[i]==0)
        {
            viz[i]=1;
            nod_start=i;
        ...
        if(nod_start==-1)//daca nu am gasit niciun nod din stanga unmatched, asta inseamna ca avem cuplajul maxim deci oprim bucla
            break;
        vector<int> drum=DFS_Cuplaj(nod_start,0,viz_right,viz_left,Right_match,solutie);//retinem in vectorul drum un drum augumentat

        if(drum.size()!=0)//daca acest drum este valid
        {
            Right_match[drum[0]]=1; //deoarece vectorul drum este inversat, asta inseamna ca primul nod este un nod unmatched din dreapta asa ca il notam ca fiind matched
            int check=0;//check este folosit pentru a verifica daca suntem in dreapta sau in stanga (0 = stanga, 1 = dreapta), la inceput suntem in stanga pornind din capatul stang al vectorului
            for(int i=drum.size()-1; i>0; i--)//parcurgem invers vectorul
            {
                int nod_1=drum[i],nod_2=drum[i-1];//retinem primul nod si al doilea nod
                if(check==0)//daca suntem in stanga atunci muchia e de forma solutie[nod_1][nod_2] , si aceasta devine muchie matched
                {
                    solutie[nod_1][nod_2]=1;//muchia devine matched
                    contor++;
                    check=1;//checkul devine 1 ca sa ne dam seama ca am trecut in partea dreapta
                    //cout<<"am scos muchia "<<nod_1<<" "<<nod_2<<"\n";
                }
                else//daca suntem in dreapta atunci muchia e de forma solutie[nod_2][nod_1] deoarece merge din dreapta in stanga si muchia aceasta devine unmactched
                {
                    solutie[nod_2][nod_1]=0;//muchia devine umatched
                    //cout<<"am adaugat muchia "<<nod_1<<" "<<nod_2<<"\n";
                    contor--;
                    check=0;//checkul devine 0 ca sa ne dam seama ca am trecut in partea stanga
                }
            }
        }
